...
const secondPromise = new Promise((resolve)=>{
    const secondPromiseHandler = (eSecond)=>{
        if (eSecond.button === 0 || eSecond.button === 2) resolve(`Second promise was resolved`);
    };
...
const thirdPromise = new Promise((resolve)=>{
    document.addEventListener("mouseup", (eDown)=>{
        if (eDown.button === 0) leftButtonDown = true;
        if (eDown.button === 2) rightButtonDown = true;
        if (leftButtonDown && rightButtonDown) resolve(`Third promise was resolved`);
    });
...
firstPromise.then((payload)=>messenger(payload, "success"), (error)=>messenger(error.message, "error"));
